...
- ES IMPORTANTE TOMAR EN CUENTA QUE PARA EJECUTAR LOS SIGUINETES COMANDOS EN LA TERMINAL, ES NECESARIO UBICARSE  EN EL MISMO DIRECTORIO DONDE SE HA COPIADO EL REPOSITORIO,
  Para movilizarse hasta ahi, se recomienda utilizar  el comando ``` sh cd  ```.


0.2  DE LA MISMA FORMA   DEBERA CREARSE UN AMBINETE VIRTUAL .venv para acceder asi, a  un espacio en el que nuestro codigo,
pueda ser ejecutado sin ningun error de dependencia y obtener los resultados mas  precisos y homogeneos.

``` sh 
 python -m venv .venv
```
se crea el ambiete VIRTUAL
...
``` 
 ...
 0.3 Luego se instalar el  archivo de requerimientos para trabajar con las mismas dependecnias  utilizadas al ejecutar el codigo.

0.4 Luego deberemos primero mostar el contenido de nuestrois archivos, y  ejecutar cada uno de los estos desde el directorio de nuestro repositorio.
...
```
UNA VEZ UBICADOS AQUI, TAMBIEN SERA NECESARIO  CREAR UN ARCHIVO `.env`, para la coneccion a la base de datos en el proyecto de SQL. Llenando las siguientes variables para crear el motor de busqueda;
...
Asi como utlizar la libreria dotenv; que se encuantra instalada en el archivo de requerimientos para acceder a la funcion load_dotenv() para caragar la variables de ambiente,
accediendo al archivo `.env`, con nuestras credenciales,nuentras credenciales.

Para utilizar la  funcion load_dotenv(), y cargar las varieables de ambiente creadas previamente en el archivo `.env`,
mediante el modulo create engine de la libreria sqlalchemy
...
from dotenv import load_dotenv()
 ...
 sera importante que llenemos nuestras credenciales en el arcivo de variables de ambiente .env mencionado con aterioridad. Especifiicando, 
 ademas de su usuario y comtraseña,tambien la direccion del servidor de la base de datos, o Host. 
 El numero de puerto en el que se esta ecuchando el servidor de la base de datos,
 el numero de puerto en el que se esta ecuchando el servidor de la base de datos, asi como el nommbre de la misma, en la  siguinete estructura:
...
tripleten_sql_db=""
```
El atchivo conectyion_t.py, cuenta con una consulta de prueba que traera las 5 primeas filas de la base de datos al ser ejecutada. Esto fue implementado en este archivo,
...
print(query)
```
si el ressultado de esta linea de codigo son las 5 primeras filas de nuestra base de datos. Entonces podremos pasar a ejecutar el siguiente archivo de datos.
...
Despues de haber comprobado nuestro arcchivo de coneccion a la base de datos funsiona,
...
asi como ta,mbien el resto de consultas a realizar en la tabla de datos respondiendo los siguinetes requerimientos.
 ...
 EN ESTE APARTADO LAS HABILIDADES CON SQL ADQUIRIDAS EN PROYECTOS Y LECIIONES, SON PUESTAs A PRUEBA CON REQUERIMIENTOS REALES DE UNA STARTUP,
 TENER UNA EXCELENTE CRECIEMINETO Y MEJORANDO EL FUNCIONAMINETO DE LA MISMA.
...
```
Entonces podremos pasar a ejecuatar nuestro archivo de test A/B, el cual tiene como objetivo econtrar un resultado de comportamiento diferente,
al grupo de control en los usuarios al que se les aplico el test. Aunque se esta retomando un proyecto ateriomente postulado, debemos describir nuestros obejetivos 
y realizar un analisis expoloratorio de datos. Slo asi podremos asegurarnos que tendremos datos correctos. Para seguir con el archivo de ejecusion lo siguinete sera;

- Estudiar la conversión en las diferentes etapas del embudo

- Evaluar los resultados de la prueba A/B

- Describir tus conclusiones con respecto a la etapa EDA y los resultados de la prueba A/B
...
MEDIANTE UNA DESCRIPCION EN MARKDOWN BACO CADA CELDA DE EJECUSION DEL CODIGO.
...
utilizaremos el analisis exploratorio de datos EDA, y algunas graficas para obtener nuestras propias conslusiones meediante los siguinetes procesos:
